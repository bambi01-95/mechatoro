...
def print_text(settext,outtext):
    img = np.zeros((100,FrameSize,3), np.uint8)                                   #check
...
def main():
    ...
    screen = pygame.display.set_mode((FrameSize + FrameSize,FrameSize+100))                            #check
    ...
    #　説明書のインポート（同じディレクトリーにないと無理）
    test = cv2.imread('inst1.png')
    nstruction = cv2.resize(test,(FrameSize,FrameSize+100))
